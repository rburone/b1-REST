/* eslint-disable */
class b1REST {
    constructor ({ URL, httpService, timeout = 5000, raw = false, debug = false }) {
        this.URL             = URL
    ...
    createEndPointCtrl(model, endPointDef = false) {
        ...
        if (endPointDef) {
            switch (typeof endPointDef) {
                ...
                case 'object':
                    if (!this[endPointDef.name]) {
                        endPointConfig = endPointDef // TODO: Validación de definicion.
            ...
            if (!exists && endPointConfig.path) {
                ...
                // ------- DEFINICION METODOS DE LA INSTANCIA
                ...
                model[endPoint] = options => {
                    ...
                    if (this.debug) {
                        ...
                        console.debug('OPTIONS: %s', JSON.stringify(options))
                    }
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        this.httpService[endPointConfig.verb](this.auth(URL), data, header).then(result => {
                            if (this.raw) {
                                result.data = result.request.response
                            }
                            ...
                            console.log('Timeout: ', !flagTimeOut)
                            if (!flagTimeOut) {
